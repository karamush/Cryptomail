...
         updateTips ("Length" + n + "should be not less than" + min + ".");
         return false;
       } else
if (o.val (). length> max) {
         o.addClass ("ui-state-error");
         updateTips ("Length of" + n + "must be less than" + max + ".");
         return false;
       }
else {
         return true;
...
$( "#registration" ).button({
      ...
      icons: {
		...
		updateTips('incorrect Email');enbauk=0;return;}
		$.ajax({
		url: 'actions.php',type: "POST",data:{ 'act': '5','mail':Base64.encode(mail.val()),'cap':captha.val()},         
		dataType : "text",                    
		success: function (data, textStatus) {
		if(data=='not'){
		updateTips('incorrect captcha');
		$("#capreg").attr( "src" , "captchareg.php?sid="+Math.random(10000, 99999) );enbauk=0;return;
		}		
		if(data=='mail'){
		updateTips('This email is already registered.');
		$("#capreg").attr( "src" , "captchareg.php?sid="+Math.random(10000, 99999) );enbauk=0;return;
		}
		$('#form').html('<h3>Registration is successful, it is expected activation.</h3>');				
		}});
	});
